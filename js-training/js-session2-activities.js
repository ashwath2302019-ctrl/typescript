...
const read = document.getElementById("name-input");
read.addEventListener('keydown',check);
...
const cd = document.getElementById("name-change");
cd.addEventListener('click',ch);
function ch(){
...
const self = async()=> {
  const[useres,postes]= await Promise.all([ 
    fetch("https://jsonplaceholder.typicode.com/users/1"),
   fetch("https://jsonplaceholder.typicode.com/posts?userId=1"),
  ]);
  const ab = await useres.json();
  const cd = await postes.json();
  
  console.log(ab.name  + " has " + cd.length + " posts");
...
const gq = document.getElementById("title");
gq.innerHTML = "<h1> Hello,Ashwath </h1>";

localStorage.setItem("Name:","Ashwath");
const local = localStorage.getItem("Name:");
console.log(local);


const bu = document .getElementById("new");
bu.addEventListener('click',load);
let controller;
async function load(){
...
const fet = await fetch("https://jsonplaceholder.typicode.com/users/1",
  {
  signal:controller.signal
});
const userr = await fet.json();
console.log(userr);
  } catch(error){
    console.log("request abort");
  }
}
const gett = document.getElementById("cancel-id");
gett.addEventListener('click',loa);
function loa(){
  controller.abort();
}
