...
- Name: locators-actions.spec.ts >> Locator Chaining and Filtering >> counts only the cards that show Pass status
- Location: tests\locators-actions.spec.ts:107:5
...
Locator:  getByTestId('intern-list').locator(':scope > div').filter({ has: getByText('Status: Pass', { exact: true }) })
Expected: 3
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for getByTestId('intern-list').locator(':scope > div').filter({ has: getByText('Status: Pass', { exact: true }) })
