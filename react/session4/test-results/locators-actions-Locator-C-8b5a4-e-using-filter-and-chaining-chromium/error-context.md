...
- Name: locators-actions.spec.ts >> Locator Chaining and Filtering >> finds Priya's score using filter and chaining
- Location: tests\locators-actions.spec.ts:85:5
...
Locator: getByTestId('intern-list').locator(':scope > div').filter({ has: getByRole('heading', { name: 'Priya', exact: true }) })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByTestId('intern-list').locator(':scope > div').filter({ has: getByRole('heading', { name: 'Priya', exact: true }) })
